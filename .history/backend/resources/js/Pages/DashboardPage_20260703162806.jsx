      ...
      <div style={styles.chartCard}>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={history.slice(-144)} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#183350" /> {/* Darkened Grid Lines */}
            <XAxis 
              dataKey="ts" 
              tickFormatter={v => new Date(v).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} 
              tick={{ fontSize: 11, fill: '#bcccdc' }} 
            />
            <YAxis tick={{ fontSize: 11, fill: '#bcccdc' }} />
            <Tooltip 
              contentStyle={{ background: '#102a43', border: '1px solid #183350', borderRadius: 8, color: '#f8fafc' }}
...
const styles = {
  page:        { color: '#f8fafc', fontFamily: 'Montserrat, sans-serif' },
  title:       { margin: '0 0 4px', color: '#f8fafc', fontSize: 22, fontWeight: 'bold' },
  ts:          { margin: '0 0 20px', color: '#bcccdc', fontSize: 12 },
  gaugeGrid:   { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16, marginBottom: 20 },
  gaugeCard:   { background: '#102a43', borderRadius: 10, padding: 20, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', cursor: 'pointer', border: '1px solid #183350', transition: 'border 0.2s' },
  gaugeCardActive: { border: '1px solid #159ed5', boxShadow: '0 0 8px rgba(21, 158, 213, 0.3)' },
  gaugeLabel:  { fontSize: 12, color: '#bcccdc', marginBottom: 8 },
  gaugeValue:  { fontSize: 32, fontWeight: 'bold', color: '#f8fafc', lineHeight: 1 },
  gaugeUnit:   { fontSize: 14, fontWeight: 'normal', marginLeft: 4, color: '#bcccdc' },
  ...
  statusCard:  { background: '#102a43', borderRadius: 10, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.1)', border: '1px solid #183350' },
  statusDot:   { width: 12, height: 12, borderRadius: '50%', flexShrink: 0 },
  statusLabel: { fontSize: 12, color: '#bcccdc' },
  statusValue: { fontWeight: 'bold', fontSize: 14 },
  chartCard:   { background: '#102a43', borderRadius: 10, padding: 20, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', border: '1px solid #183350' },
  ...
  tab:         { padding: '4px 10px', borderRadius: 6, border: '1px solid #183350', background: '#0c1a30', cursor: 'pointer', fontSize: 12, color: '#bcccdc', fontFamily: 'Montserrat, sans-serif' },
