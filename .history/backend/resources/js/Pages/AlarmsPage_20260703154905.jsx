/**
 * OPMAS-001 | Alarms Page
 * Lists active and resolved alarms. Operators can resolve active alarms.
 */
import React, { useState, useEffect } from 'react';
import { getAlarms, resolveAlarm } from '../services/api';

const SEVERITY_STYLE = {
  CRITICAL: { bg: '#fee2e2', color: '#991b1b', border: '#fca5a5' },
  WARNING:  { bg: '#fef3c7', color: '#92400e', border: '#fcd34d' },
  INFO:     { bg: '#dbeafe', color: '#1e40af', border: '#93c5fd' },
};

export default function AlarmsPage() {
  const [alarms,  setAlarms]  = useState([]);
  const [loading, setLoading] = useState(true);
  const [filter,  setFilter]  = useState('active'); // active | all

  const load = async () => {
    try {
      const data = await getAlarms();
      setAlarms(data.alarms || []);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { load(); const t = setInterval(load, 15_000); return () => clearInterval(t); }, []);

  const handleResolve = async (id) => {
    await resolveAlarm(id);
    load();
  };

  const visible = filter === 'active'
    ? alarms.filter(a => !a.resolved_at)
    : alarms;

  const activeCount = alarms.filter(a => !a.resolved_at).length;

  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Alarms</h1>
          {activeCount > 0 && <p style={styles.activeCount}>{activeCount} active alarm{activeCount > 1 ? 's' : ''}</p>}
        </div>
        <div style={styles.tabs}>
          {['active', 'all'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ ...styles.tab, ...(f === filter ? styles.tabActive : {}) }}>
              {f === 'active' ? 'Active' : 'All History'}
            </button>
          ))}
        </div>
      </div>

      {loading && <div style={styles.empty}>Loading alarms...</div>}
      {!loading && visible.length === 0 && (
        <div style={styles.empty}>
          {filter === 'active' ? '✅ No active alarms' : 'No alarms recorded'}
        </div>
      )}

      <div style={styles.list}>
        {visible.map(alarm => {
          const s = SEVERITY_STYLE[alarm.severity] || SEVERITY_STYLE.INFO;
          const resolved = !!alarm.resolved_at;
          return (
            <div key={alarm.id} style={{ ...styles.card, background: s.bg, borderColor: s.border, opacity: resolved ? 0.7 : 1 }}>
              <div style={styles.cardLeft}>
                <div style={styles.cardTop}>
                  <span style={{ ...styles.badge, color: s.color }}>{alarm.severity}</span>
                  <span style={styles.type}>{alarm.type.replace(/_/g, ' ')}</span>
                  {resolved && <span style={styles.resolvedBadge}>RESOLVED</span>}
                </div>
                <p style={styles.message}>{alarm.message}</p>
                <div style={styles.timestamps}>
                  <span>Triggered: {new Date(alarm.triggered_at).toLocaleString()}</span>
                  {alarm.resolved_at && <span> · Resolved: {new Date(alarm.resolved_at).toLocaleString()}</span>}
                </div>
              </div>
              {!resolved && (
                <button style={styles.resolveBtn} onClick={() => handleResolve(alarm.id)}>
                  Resolve
                </button>
              )}
            </div>
          );
        })}
      </div>
    </div>
  );
}

const styles = {
  page:          { padding: 24 },
  header:        { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
  title:         { margin: 0, color: '#1b3a6b', fontSize: 22 },
  activeCount:   { margin: '4px 0 0', color: '#dc2626', fontSize: 13, fontWeight: 'bold' },
  tabs:          { display: 'flex', gap: 8 },
  tab:           { padding: '6px 16px', borderRadius: 6, border: '1px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', fontSize: 13 },
  tabActive:     { background: '#1b3a6b', color: '#fff', border: '1px solid #1b3a6b' },
  empty:         { textAlign: 'center', padding: 60, color: '#64748b', fontSize: 15 },
  list:          { display: 'flex', flexDirection: 'column', gap: 12 },
  card:          { border: '1px solid', borderRadius: 10, padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 },
  cardLeft:      { flex: 1 },
  cardTop:       { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 },
  badge:         { fontWeight: 'bold', fontSize: 12, textTransform: 'uppercase' },
  type:          { fontWeight: 'bold', color: '#1e293b', fontSize: 14 },
  resolvedBadge: { background: '#dcfce7', color: '#166534', fontSize: 11, padding: '2px 8px', borderRadius: 99, fontWeight: 'bold' },
  message:       { margin: '0 0 6px', color: '#374151', fontSize: 13 },
  timestamps:    { color: '#6b7280', fontSize: 11 },
  resolveBtn:    { padding: '6px 16px', background: '#1b3a6b', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13, whiteSpace: 'nowrap' },
};
